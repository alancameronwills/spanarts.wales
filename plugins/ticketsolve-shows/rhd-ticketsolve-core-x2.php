  ...
  $locale = "?locale=" . ($cymraeg ? "cy-CY" : "en-GB");
	foreach ($showlist as $show) {
    	...
    	$tags = count($show['tags']) > 0 ? " " . implode(' ', $show['tags']) : "";
		  $date = "";
      if (isset($show['event']['dtstart'])) {
        if ($cymraeg) {
          $dateString = $show['event']['dtstart'];
          $dayOfWeek = intval(date("w", $dateString));
          $dayOfMonth = intval(date("j", $dateString));
          $monthIndex= date("n", $dateString);
          ...
          $date = $dayName . " " . $dayOfMonth . " " . $monthName . " " . date('Y H:i', $dateString);
        }
        else {
          $date = date('D jS M Y H:i', $show['event']['dtstart']);
        }
      }
