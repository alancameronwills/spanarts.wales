<!-- Span Arts - blog_single-loop.php -->
<?php 
/* This is the Default template. It deals with a number of post formats - video, standard, etc
*/
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<div class="post_content_holder">
				<div class="post_image">
					<?php $bridge_qode_video_type = span_extract_youtube_id(get_post_meta(get_the_ID(), "video_format_choose", true));?>
					<?php if($bridge_qode_video_type == "youtube") { 
						$video_id = get_post_meta(get_the_ID(), "video_format_link", true);
					?>
						<iframe name="fitvid-<?php the_ID(); ?>" src="//www.youtube.com/embed/<?=$video_id?>?wmode=transparent" wmode="Opaque" width="805" height="403" allowfullscreen></iframe>
				...
				<div class="post_text">
					<div class="post_text_inner">
						...
						<div class="post_info">
							...
							<span itemprop="dateCreated" class="date entry_date updated"><?php the_time('d M'); ?><meta itemprop="interactionCount" content="UserComments: <?php echo get_comments_number(bridge_qode_get_page_id()); ?>"/></span>
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<div class="post_content_holder">
				<?php if(get_post_meta(get_the_ID(), "qode_hide-featured-image", true) != "yes") {
					$toppic = get_field("toppic");
					if ($toppic) {
						// https://www.advancedcustomfields.com/resources/image/
						?>
						<div class="post_image">
	                        <?php echo wp_get_attachment_image( $toppic, 'full' ); ?>
						</div>
						<?php
				...
				<div class="post_text">
					<div class="post_text_inner">
						<h2 itemprop="name" class="entry_title"><meta itemprop="interactionCount" content="UserComments: <?php echo get_comments_number(bridge_qode_get_page_id()); ?>"/></span> <?php the_title(); ?></h2>
						<div class="post_info">
							<span class="time"><?php esc_html_e('Posted at','bridge'); ?> <?php the_time('H:i'); ?><?php esc_html_e('h','bridge'); ?>  <span itemprop="dateCreated" class="date entry_date updated"><?php the_time('d M Y'); ?></span>
							</span>
							<span class="post_categories"><?php esc_html_e('in','bridge'); ?> <?php the_category(', '); ?></span>
							<span class="post_author">
								<?php esc_html_e('by','bridge'); ?>
								<a itemprop="author" class="post_author_link" href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>"><?php the_author_meta('display_name'); ?></a>
